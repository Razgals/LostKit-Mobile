...
String buildWorldUrl(int worldId, bool highDetail) {
  final detail = highDetail ? 'high' : 'low';
  return 'https://2004.lostcity.rs/client?world=$worldId&detail=$detail&method=0';
}
...
class _WorldSwitcherPanelState extends State<WorldSwitcherPanel> {
  ...
  Future<void> _measureLatency(World world) async {
    try {
      final pingUrl = buildWorldUrl(world.id, true);
      final start = DateTime.now().millisecondsSinceEpoch;
      await http.head(Uri.parse(pingUrl)).timeout(const Duration(seconds: 5));
              ...
              : _error
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        final url = buildWorldUrl(world.id, _highDetail);
                        final label =
                            'W${world.id} ${_highDetail ? 'HD' : 'LD'}';
                        return _WorldTile(
                            world: world,
                            onTap: () =>
                                widget.onWorldSelected(url, label));
                      },
                    ),
        ),
      ],
      ...
      child: Container(
        ...
        child: Text(label,
            style: TextStyle(
              ...
              fontSize: 11,
              color:
                  active ? Colors.white : const Color(0xFF888888),
              fontWeight: active
                  ? FontWeight.bold
                  : FontWeight.normal,
            )),
